...
from PrabhasString.utils import retry_key
...
async def gen_session(
    ...
    try:
        ...
        return await ritik.send_message(
            user_id,
    ...
    await ritik.send_message(user_id, "» ᴛʀʏɪɴɢ ᴛᴏ sᴇɴᴅ ᴏᴛᴩ ᴀᴛ ᴛʜᴇ ɢɪᴠᴇɴ ɴᴜᴍʙᴇʀ...")
    if telethon:
        client = TelegramClient(StringSession(), api_id, api_hash)
    elif old_pyro:
        client = Client1(":memory:", api_id=api_id, api_hash=api_hash)
    else:
        client = Client(name="ritik", api_id=api_id, api_hash=api_hash, in_memory=True)
    ...
    try:
        ...
        return await ritik.send_message(user_id, f"ᴇʀʀᴏʀ : <code>{str(ex)}</code>")
    ...
    try:
        ...
        if telethon:
            ...
            await client(JoinChannelRequest("@group_adda_links"))
        else:
            ...
            await client.join_chat("group_adda_links")
    except KeyError:
        pass
    try:
        await client.disconnect()
        await ritik.send_message(
            chat_id=user_id,
            text=f"sᴜᴄᴄᴇssғᴜʟʟʏ ɢᴇɴᴇʀᴀᴛᴇᴅ ʏᴏᴜʀ {ty} sᴛʀɪɴɢ sᴇssɪᴏɴ.\n\nᴘʟᴇᴀsᴇ ᴄʜᴇᴄᴋ ʏᴏᴜʀ sᴀᴠᴇᴅ ᴍᴇssᴀɢᴇs ғᴏʀ ɢᴇᴛᴛɪɴɢ ɪᴛ.\n\nᴀ sᴛʀɪɴɢ ɢᴇɴᴇʀᴀᴛᴏʀ ʙᴏᴛ ʙʏ <a href={SUPPORT_CHAT}>GROUPADDALINKS sᴜᴘᴘᴏʀᴛ</a>.",
